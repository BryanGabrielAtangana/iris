...
export const HARD_CASES: EvalCase[] = [
  // git-commit-message
  { query: "describe the edits I just staged so they land in source control", expected: "git-commit-message" },
  { query: "what wording should accompany this set of changes when I save it to the repo history", expected: "git-commit-message" },
  ...
  // changelog-update
  { query: "note this new capability in the project's release history file", expected: "changelog-update" },
  { query: "record what changed under the upcoming version section", expected: "changelog-update" },
  { query: "add a user-facing bullet about this fix to the changes log", expected: "changelog-update" },

  // regex-builder
  { query: "craft a pattern that recognizes valid email-looking strings", expected: "regex-builder" },
  { query: "my expression for matching phone numbers isn't catching them, help me fix it", expected: "regex-builder" },
  ...
  { query: "speed up a slow query by adding the right index, as a schema change", expected: "sql-migration" },
  ...
  // api-mock-server
  { query: "spin up a throwaway endpoint that returns canned responses for my tests", expected: "api-mock-server" },
  { query: "fake a backend so my front-end tests don't hit the real service", expected: "api-mock-server" },
  { query: "serve some hardcoded JSON locally while I develop", expected: "api-mock-server" },

  // dockerfile-author
  { query: "package my node service into a small reproducible image", expected: "dockerfile-author" },
  { query: "shrink the size of the container build for my app", expected: "dockerfile-author" },
  { query: "write the build recipe to run my python app in a container", expected: "dockerfile-author" },
];
